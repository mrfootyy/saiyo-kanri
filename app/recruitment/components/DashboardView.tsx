          ...
          <ul className="divide-y divide-blue-100">
            {tasks.map((task) => (
              <li key={`${task.candidateId}-${task.kind}-${task.stageId || task.stageName}`}>
                ...
                >
                  <svg className="h-4 w-4 flex-shrink-0 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              ...
              recentSlack.map((n) => (
                <li key={n.id} className="px-5 py-3.5">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-xs font-semibold text-[#4A154B]">{n.channel}</span>
